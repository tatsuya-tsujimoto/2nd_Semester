...
int main()
{
	...
	// dynamic_cast<変化する型名>(変化元の値(変数))
	Enemy01 enemy01;
	CharacterBase* pBase = &enemy01;		// 基底のポインタ変数へ、派生のアドレス代入
//	Enemy00* pEnemy00 = pBase;			// 派生ポインタ変数へ、基底のアドレスを代入(error)
...
//	pEnemy01->SetRoutine(5);
	...
	// 基底クラスに、派生暗さうが何であるか判別するための変数などを用意して使用する
	switch (pBase->GetEnemyType())
	{
	case EnemyType_Weak00:
		pEnemy00 = reinterpret_cast<Enemy00*>(pBase);
		break;
	case EnemyType_Weak01:
